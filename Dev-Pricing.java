...
operationName                        = "";
zouraErrorMessage                    = "";
zuoraSubscriptionIdStrForCreditLines = "";
...
if(quoteType_temp == "Auto-Renew" AND (_system_user_login <> "sonusharma" AND _system_user_login <> "surajmutha" AND _system_user_login <> "APIuser" AND _system_user_login <> "sgaddam" AND _system_user_login <> "daveespie") AND (_system_current_step_var=="start_step" OR _system_current_step_var=="pending_process"))
{
...
for line in transactionLine {
    ...
    if(line.assetID_l <> "")
    {
        ...
        if(line.lineType_line == "credit" OR line.lineType_line == "cancel")// CRM-811 added by Suraj to get total billed amount and total credit for each asset
        {
            if(zuoraSubscriptionIdStrForCreditLines <> "")
            {
                zuoraSubscriptionIdStrForCreditLines = zuoraSubscriptionIdStrForCreditLines + ",";
            }
            zuoraSubscriptionIdStrForCreditLines = zuoraSubscriptionIdStrForCreditLines + line.zuoraSubscriptionID_line;
        }
    }
...
zuoraTotalBilledAmountDict = dict("string");
zuoraTotalCreditAmountDict = dict("string");
if(zuoraSubscriptionIdStrForCreditLines <> "" AND (_system_current_step_var=="start_step" OR _system_current_step_var=="pending_process"))// CRM-811
{
    returnDict = dict("dict<string>");
    inputDictForBilledAmount = dict("string");
    put(inputDictForBilledAmount,"session",session);
    put(inputDictForBilledAmount,"endPoint",endPoint);
    put(inputDictForBilledAmount,"subscriptionIDStr",zuoraSubscriptionIdStrForCreditLines);
    returnDict = commerce.getBilledAmountAndTotalCredit(inputDictForBilledAmount);
    if(containsKey(returnDict,"totalChargeAmountDict"))
    {
        zuoraTotalBilledAmountDict = get(returnDict,"totalChargeAmountDict");
        zuoraTotalCreditAmountDict = get(returnDict,"totalCreditDict");
    }
}
...
for line in transactionLine {
    ...
    if (partNumber <> "" )
    {
        ...
        listPriceOverride = line.listPriceOverride_line;
        ...
        if(line.assetDetails_line <> "")
        {
            if(lineType == "renew" AND lineType <> oldLineType)
            {
				...
				listPriceOverride = 0.0;
				billingPeriod = assetArray[BILLING_PERIOD_INDEX];
				if(actualContractTerm == 6 AND billingPeriod == "Annual"){
					billingPeriod = "Bi-Annual";
				}elif(actualContractTerm == 3 AND (billingPeriod == "Annual" OR billingPeriod == "Bi-Annual")){
					billingPeriod = "Quarterly";
				}
				ContractTerm = actualContractTerm;
				//lineDiscount = 0.0;
				lineRes = lineRes + documentNumber + "~overrideTerm_line~" + string(0) + "|"
            ...
            elif(lineType <> "renew" AND oldLineType == "renew")
            {
        ...
        totalBilledAmt = 0;// CRM-811
        totalBilledCreditAmt = 0;
        if((lineType == "credit" OR lineType == "cancel") AND containsKey(zuoraTotalBilledAmountDict,line.zuoraSubscriptionID_line))
        {
            totalBilledAmt = atof(get(zuoraTotalBilledAmountDict,line.zuoraSubscriptionID_line));
            if(containsKey(zuoraTotalCreditAmountDict,line.zuoraSubscriptionID_line))
            {
                totalBilledCreditAmt = atof(get(zuoraTotalCreditAmountDict,line.zuoraSubscriptionID_line));
            }
        }
        lineRes = lineRes + documentNumber + "~totalBilledAmount_line~" + string(totalBilledAmt) + "|"
                  + documentNumber + "~totalBilledCreditAmount_line~" + string(totalBilledCreditAmt) + "|";// end CRM-811
        ...
        /*if(line.contractTerms_line <> "" AND isNumber(line.contractTerms_line))// term can be changed on changing line action CRM-811 so commented this code
        {
        ...
        if(line._part_number == "ADVANTAGE" AND line.pricingStringForAdvantage_line <> ""){
            ...
            if(sizeofarray(selectedPartArr) > 2){
                ...
                if(billingPeriod=="Monthly" AND isnumber(selectedPartArr[1]))
                {
                    listPrice = atof(selectedPartArr[1]);
                ...
                elif(containsKey(billingPeriodDivisorDict,billingPeriod))
                {
                    listPrice = atof(selectedPartArr[2]) * get(billingPeriodDivisorDict,billingPeriod);
                    if(isNull(selectedPartArr[3]) == false)
                    {
                        originalListPriceForDelta = atof(selectedPartArr[4]) * get(billingPeriodDivisorDict,billingPeriod);
                    }
                }
            }
        }
        ...
        discountAmount = 0;
        if(listPriceOverride > 0)
        {
            discountAmount = listPrice - listPriceOverride;
        }
        ...
        if(listPriceOverride > 0){
            ...
            netPriceEach = listPriceOverride - discountAmountEach;
        }else{
        ...
        totalValueLine = 0;
        totalEstTaxLine = 0;
        if(containsKey(billingPeriodDivisorDict,billingPeriod))//line.contractTerms_line <> "" AND
        {
            totalListPriceLine = listPrice * qty * ceil(ContractTerm/get(billingPeriodDivisorDict,billingPeriod));
            totalDiscountLine = discountAmount * qty * ceil(ContractTerm/get(billingPeriodDivisorDict,billingPeriod));
        ...
        elif(partNumber == "TURBO")
        {
            ...
            if((actionName == "update") AND (lineType == "cancel" OR lineType == "credit"))//actionName == "submit" OR
            {
                ...
                if(listPriceOverride > 0)
                {
                    listPriceValue = listPriceOverride;
                }
        ...
        if(listPrice <> line.originalListPrice_line AND line.originalListPrice_line > 0 AND NOT (lineType <> "renew" AND oldLineType == "renew"))// reset over ride amount if user modified any line level attribute which causes change in original list price
        {		
        ...
        totalBillingTerms = 0;
        if(containsKey(billingPeriodDivisorDict,billingPeriod))
        {
            totalBillingTerms = ContractTerm/get(billingPeriodDivisorDict,billingPeriod);
        }
        ...
        if(listPriceOverride <> 0 AND (lineType == "add" OR lineType == "renew") AND (listPrice <> listPriceOverride)){

            overrideDelta = (listPrice - listPriceOverride)*totalBillingTerms*qty;
            if(overrideDelta < 0){
        ...
        if(line.assetDetails_line <> "" AND undersoldAsset == true){
            ...
            put(lineDetailsDict,"listBillingPeriod",billingPeriod);
            if(listPriceOverride <> 0){
                put(lineDetailsDict,"listPrice",String(listPriceOverride));
            }else{
        ...
        if((_system_current_step_var=="start_step" OR _system_current_step_var=="pending_process")){
            ...
            for eachPromo in promoresults{
                        ...
                        if((get(eachPromo,"SoldWithPart") == "NONE" OR findinarray(soldWithPartNumber,get(eachPromo,"SoldWithPart")) <> -1) AND (get(eachPromo,"SoldWithType") == "NONE" OR findinarray(soldWithProdType,get(eachPromo,"SoldWithType")) <> -1) AND (tempLineAction == "ALL" OR find(tempLineAction,lineType) <> -1) OR (line._part_number == "FIVESTREET" AND find(parentAssetMLSString_quote,"#$#COBROKE#$#") <> -1 AND promoCode == "FSFREECBC")){
                            ...
                            if(promoCode == "C21BRKRSHOWCASE"){
                                if(line._part_number == "SHOWCASE" AND lower(accountType) == "broker" AND (lower(trim(franchise)) == "century 21" OR lower(franchise) == "c21")){
                                    ...
                                    if(billingPeriod <> "" AND contractTerm > 0){
                                        if(billingPeriod=="Monthly"){
                ...
                if((quoteType_temp == "Auto-Renew" OR oldLineType <> lineType) AND lineType == "renew")
                {
            ...
            if((lineType == "amend" OR lineType == "credit") AND line.assetDetails_line  <> ""){
                ...
                //assetArray    = split(line.assetDetails_line, FIELD_DELIM);
                if(assetArray[PROMOTION_INDEX] <> "" AND find(eligiblePromotionString,assetArray[PROMOTION_INDEX]+promoDelim) == -1 AND containsKey(promoApplicationdct,assetArray[PROMOTION_INDEX]+ "@PromoCode")){
                    currentPromo = assetArray[PROMOTION_INDEX];
        ...
        if((quoteType_temp == "Auto-Renew" OR oldLineType <> lineType) AND lineType == "renew" AND (_system_current_step_var=="start_step" OR _system_current_step_var=="pending_process"))
        {
        ...
        /* if(listPriceOverride > 0 AND (listPrice > listPriceOverride OR line._part_custom_field9 <> "Tigerleads") AND (lineType == "add" OR lineType == "renew"))
        {
            append(approvalReasonArray,"ListPriceOverride");
        } */
        //LEAP-8924 //CRM-846
        if(listPriceOverride > 0.0 AND (listPrice > listPriceOverride) AND (lineType == "add" OR lineType == "renew") AND isSpecialAdvantage == false)
        {
            append(approvalReasonArray,"ListPriceOverride");
        }elif(lineType == "amend" OR lineType == "credit"){
            //assetArray    = split(line.assetDetails_line, FIELD_DELIM);
            if(isNumber(assetArray[LSIT_PRICE_OVERRIDE_INDEX])){
                ...
                if( assetListPriceOverride <> 0 AND listPriceOverride > 0 AND assetListPriceOverride > listPriceOverride ){
                    append(approvalReasonArray,"ListPriceOverride");
                }elif(assetListPriceOverride == 0 AND listPriceOverride > 0.0  AND listPrice > listPriceOverride){
                    append(approvalReasonArray,"ListPriceOverride");
                }
            }elif(assetArray[LSIT_PRICE_OVERRIDE_INDEX] == ""  AND listPriceOverride > 0.0  AND listPrice > listPriceOverride){
                append(approvalReasonArray,"ListPriceOverride");
            }
        }
