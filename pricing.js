...
MARKET_KEY_INDEX = 39;
// CRM-1280
CUST_CONTRACT_TERM_INDEX=55;
CUST_LIST_PRICE=56;
CUST_PROMOTION_INDEX=57;
CUST_PROMOTION_INDEX_AGE=58;
CUST_SPOT_PROMOTION_INDEX=59;
...
for line in transactionLine {
	...
	if (partNumber <> "" )
	{
		...
		oldLineType = line.oldLineType_line; // CRM-1090
		...
		if(line.externalID_line <> "" AND line._part_custom_field13 == "true" AND actionName == "update"){
            ...
            if(lineType == "renew" AND containsKey(assetWebServiceDataDict,documentNumber))// get price tier from webservice call.
            {
                ...
                if(checkInventory == false){
                    checkInventory = true;
					// Commmented below for 1280 and added above
					/*if(quoteType_temp == "Auto-Renew" AND containsKey(thresholdDict,partNumber) AND ((marketSold > get(thresholdDict,partNumber)) OR (total_units == 0 AND total_units_unavailable > 0))){
						needManualRenew = true;
					}*/
		...
		listPrice = 0;
		listPriceOverride = line.listPriceOverride_line; // CRM-1090
		...
		if(line.assetDetails_line <> "")
		{
			if(lineType == "renew" AND lineType <> oldLineType) //CRM-1090
			{
				...
				//End Ravi
				if(isNumber(assetArray[CUST_CONTRACT_TERM_INDEX]) AND atoi(assetArray[CUST_CONTRACT_TERM_INDEX]) > 0 AND find(line._part_custom_field21,assetArray[CUST_CONTRACT_TERM_INDEX]) <> -1){ // CRM-1280
					contractTerm = atoi(assetArray[CUST_CONTRACT_TERM_INDEX]);
					
				}
				elif(line._part_custom_field22 <> "") // CRM-1090
				{
			  ...
			  // clear override term for non inventory renewals	: CRM-1090		  
			  //if(line._part_custom_field13=="false"){
			  lineRes = lineRes + documentNumber + "~overrideTerm_line~0|";
			  //}				
				// CRM-1280	: Start
                 lineRes = lineRes + documentNumber + "~contractTerms_line~" + string(ContractTerm) + "|";	//CRM-1090			
			...
			elif(lineType <> "renew" AND oldLineType == "renew") //CRM:1090
			{
				...
				//if(oldLineType == "renew" AND lineType <> "renew")// if line action is changed from renew to amend etc then reset values from asset so user can change discount or any other value after changing line action to amend
				//{
		...
		if(listPriceOverride>0) //CRM-1090
		{
        ...
        // ( listPrice_line * ( combinedDiscount_line - ( promoDiscount_line + quoteDiscount_line ) ) ) * .01 
        // 100 - ( ( ( 100 - ( promoDiscount_line + quoteDiscount_line ) ) * ( 100 - spotDiscountPercent_line ) ) / 100 )
		...
		if((_system_current_step_var=="start_step" OR _system_current_step_var=="pending_process") AND (quoteType_temp == "Retention" OR quoteType_temp == "Backout" OR quoteType_temp == "Modify" OR quoteType_temp == "Auto-Credit")){
			...
			elif(line._part_custom_field8 <> "Usage")
			{
				...
				else
				{
					...
					{
						//print inputDict;
						//print lineItemValuesDict;
						//print assetValuesDict;
		...
		if((listPrice <> line.originalListPrice_line AND (line.originalListPrice_line > 0 OR line._part_custom_field13=="false") AND NOT (lineType <> "renew" AND oldLineType == "renew")) OR NOT (lineType <> "renew" AND oldLineType == "renew"))// reset over ride amount if user modified any line level attribute which causes change in original list price
		{
		...
		if(listPriceOverride <> 0 AND (lineType == "add" OR lineType == "renew") AND (listPrice <> listPriceOverride)){ //CRM-1090
		...
		if(line.assetDetails_line <> "" AND undersoldAsset == true){
			...
			if(lineType == "renew" AND oldLineType == "renew"){
				//if (line.listPriceOverride_line <> 0 AND line.override_line >= 0)
				if (listPriceOverride <> 0 AND line.override_line >= 0)				
				{ // CRM-1128: Updated the condition
			...
			}elif(lineType == "renew" AND oldLineType <> lineType){
				put(lineDetailsDict, "listPrice", String(listPrice));
			}
			/*if(line.listPriceOverride_line <> 0 AND line.override_line >=0){ // CRM-1128: Updated the condition
		...
		if((_system_current_step_var=="start_step" OR _system_current_step_var=="pending_process")){
						...
						if((get(eachPromo,"SoldWithPart") == "NONE" OR findinarray(soldWithPartNumber,get(eachPromo,"SoldWithPart")) <> -1) AND (get(eachPromo,"SoldWithType") == "NONE" OR findinarray(soldWithProdType,get(eachPromo,"SoldWithType")) <> -1) AND (tempLineAction == "ALL" OR find(tempLineAction,lineType) <> -1) OR (line._part_number == "FIVESTREET" AND find(parentAssetMLSString_quote,"#$#COBROKE#$#") <> -1 AND promoCode == "FSFREECBC")){
							...
							if(promoCode == "C21BRKRSHOWCASE"){
								if(line._part_number == "SHOWCASE" AND lower(accountType) == "broker" AND (lower(trim(franchise)) == "century 21" OR lower(franchise) == "c21")){
									if(line.hlcOverride_line > 0){
									 ...
									 if(line.assetDetails_line <> ""){
										 ...
										 if(line.contractTermFlag_line == false){
											if(isNumber(assetArray[CUST_CONTRACT_TERM_INDEX]) AND atoi(assetArray[CUST_CONTRACT_TERM_INDEX]) > 0 AND lineType == "renew"){ // CRM-1280
												contractTerm = atoi(assetArray[CUST_CONTRACT_TERM_INDEX]);
											}
											elif(isNumber(assetArray[OVERRIDER_CONTRACT_TERM_INDEX])){
									...
									if(listPriceOverride > 0.0){	//CRM-1090								
							...
							// END CRM-1816
							    ...
							    //CRM-1526:Start
								if(promoCode == "ADVANTAGECAPPEDPRICE" AND line.hidePromoFlag_line == true){
								continue;
								}
								//CRM-1526:End
								...
								if((marketCode <> "ALL" AND marketCode == lineMarketCode)){
									...
									// CRM-1526 : Start
									if((promoCode == "ADVANTAGECAPPEDPRICE" AND line.hidePromoFlag_line == false )){							
											
											//if(NOT(containsKey(appliedPromotionDict, atoi(documentNumber))) AND lineType <> "renew"){
											if(lineType <> "renew"){
												put(appliedPromotionDict,atoi(documentNumber),promoCode);
			...
			//CRM-1280 : Start
			elif(lineType == "renew"  AND line.assetDetails_line  <> "" AND quoteType_temp <> "Auto-Renew")
			{			    			
				if(find(eligiblePromotionString,assetArray[CUST_PROMOTION_INDEX]+promoDelim) == -1 AND containsKey(promoApplicationdct,assetArray[CUST_PROMOTION_INDEX]+ "@PromoCode")){
					if(lineType == "renew" AND quoteType_temp <> "Auto-Renew" AND NOT(ISNULL(assetArray[CUST_PROMOTION_INDEX])) AND assetArray[CUST_PROMOTION_INDEX] <> ""){
						if(isnumber(assetArray[CUST_PROMOTION_INDEX_AGE]) AND atof(assetArray[CUST_PROMOTION_INDEX_AGE]) > 0){									 
										discountPercent = atof(assetArray[CUST_PROMOTION_INDEX_AGE]);
										promoPercentageForRenewal = discountPercent;
										promoCodeForRenewal = assetArray[CUST_PROMOTION_INDEX];
										put(appliedPromotionDict,atoi(documentNumber),assetArray[CUST_PROMOTION_INDEX]);
										eligiblePromotionString = eligiblePromotionString + assetArray[CUST_PROMOTION_INDEX] + promoDelim + string(round(discountPercent,2)) + promoDelim + documentNumber + menuItemDelim;										
									 }
									}
				}
		...
		if(containsKey(AppliedPromotionDict, atoi(documentNumber))){
			...
			}elif((_system_current_step_var=="start_step" OR _system_current_step_var=="pending_process") AND amendPromoSelected == false AND oldLineType == "renew" AND lineType <> "renew"){//CRM-1090
				lineRes = lineRes + documentNumber + "~override_line~" + "0.0" + "|";
			}
		}
		...
		//CRM-1280 & 1128 : commented the above line
		if(lineType == "renew" AND (_system_current_step_var=="start_step" OR _system_current_step_var=="pending_process"))
		{
		    //if(oldLineType <> lineType){ //CRM-1090
				//CRM-1280 : Start			
			...
			//CRM-1128 : Start
			if(find(applied,"PRICEGRANDFATHER") <> -1 AND promoPercentageForRenewal < 0){
				...
				lineRes = lineRes + documentNumber + "~listPriceOverride_line~0.0|";
			}elif(find(applied,"PRICEGRANDFATHER") <> -1 AND promoPercentageForRenewal >=0){ 
				if(lineType <> oldLineType OR (NOT(containsKey(AppliedPromotionDict, atoi(documentNumber))) OR find(get(AppliedPromotionDict, atoi(documentNumber)),"PRICEGRANDFATHER") <> -1 OR get(AppliedPromotionDict, atoi(documentNumber)) == "")){
				//if(lineType <> oldLineType OR (find(get(AppliedPromotionDict, atoi(documentNumber)),"PRICEGRANDFATHER") <> -1)){
					resetAppliedPromoQuote = resetAppliedPromoQuote + documentNumber + ".!." + applied + "!!!" ;
					if(promoPercentageForRenewal <> line.promotionDiscountPercent_line){
						lineRes = lineRes + documentNumber + "~override_line~" + string(promoPercentageForRenewal) + "|";
					}					
				}				
			}elif(find(applied,"PRICEGRANDFATHER") == -1){
		...
		if(documentNumber<>"1"){
			...
			if(NOT isnull(applied) AND applied<>""){
				if( containsKey(promoApplicationdct, applied+"@DiscountPercent") AND isNumber(get(promoApplicationdct, applied+"@DiscountPercent") )){
					...
					//CRM-1128
					if(find(applied,"PRICEGRANDFATHER") <> -1){
					 promotionDiscount=promoPercentageForRenewal;
					}
					// CRM-1280 : Start
					if(quoteType_temp == "Auto-Renew"){
		...
		//LEAP-8924 //CRM-846
		//CRM-1090 update listprice attribute with variable
