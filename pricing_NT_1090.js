...
for line in transactionLine {
	...
	if (partNumber <> "" )
	{
		...
		/*print("Before Reset Call");
		print("Line Discount");
		print(line.override_line);
		print("MPO");
		print(line.listPriceOverride_line); 
		print("Contract Term"); 
		print(line.contractTerms_line);
		print("Billing Period");
		print(line.billingPeriod_line); 
		//CRM-1090 Reset Call
		...
		if(line.assetDetails_line <> "" AND undersoldAsset == true){
			...
			put(lineDetailsDict,"listBillingPeriod",billingPeriod); //Added as part of CRM-1090 NT
			if(line.listPriceOverride_line <> 0){
		...
		if((_system_current_step_var=="start_step" OR _system_current_step_var=="pending_process")){
			...
			for eachPromo in promoresults{
				...
				if((quoteType_temp == "Auto-Renew" OR oldLineType <> lineType) AND lineType == "renew") // CRM:1090 NT
				//if((quoteType_temp == "Auto-Renew") AND lineType == "renew")
				{
			...
			if((lineType == "amend" OR lineType == "credit") AND line.assetDetails_line  <> ""){
				...
				if(assetArray[PROMOTION_INDEX] <> ""){
					...
					if(find(eligiblePromotionString,assetArray[PROMOTION_INDEX]+promoDelim) == -1 AND containsKey(promoApplicationdct,assetArray[PROMOTION_INDEX]+ "@PromoCode")){
						...
						//grandFathered = get(promoApplicationdct, assetArray[PROMOTION_INDEX]+"@Grandfathered");//commented for CRM-1090 NT 
						//toDate = atoi(get(promoApplicationdct, assetArray[PROMOTION_INDEX]+"@DateTo")); //commented for CRM-1090 NT 
						...
						//if((lineType == "amend") OR (lineType == "credit") OR (lineType == "renew" AND (undersoldAsset OR lower(grandFathered) == "yes") AND quoteType_temp <> "Auto-Renew")){ // commented above code and added undersoldAsset condition as part of CRM-427 so if undersoldAsset is true then promo code is valid from asset.////CRM-1090 NT
							if(containsKey(promoApplicationdct, currentPromo+"@DiscountPercent") AND isNumber(get(promoApplicationdct, currentPromo+"@DiscountPercent"))){
									discountPercent = atof(get(promoApplicationdct,currentPromo+"@DiscountPercent"));
							}elif(assetArray[MANUAL_DISCOUNT_TYPE_INDEX] <> "" AND isNumber(assetArray[MANUAL_DISCOUNT_TYPE_INDEX])){
									discountPercent = atof(assetArray[MANUAL_DISCOUNT_TYPE_INDEX]);
		...
		//if((quoteType_temp == "Auto-Renew" OR oldLineType <> lineType) AND lineType == "renew" AND (_system_current_step_var=="start_step" OR _system_current_step_var=="pending_process")) //CRM:1090 NT CHECK
		//CRM-1280
